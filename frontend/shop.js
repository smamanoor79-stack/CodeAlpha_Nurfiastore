...
import { addToBackendWishlist, removeFromBackendWishlist, isLoggedIn } from './api.js';
...
function toggleWishlistShop(id, btn, product) {
...
function renderProducts() {
  ...
  : p.video
    ...
    : `<img src="${p.image}" alt="${p.name}" />`
        ...
        <div class="shop-card-actions">
          <button class="shop-action-btn shop-wishlist" title="Wishlist" style="${inWishlist ? 'color:#e00;' : ''}">
  ...
  grid.querySelectorAll('.shop-card').forEach(card => {
    ...
    const product = paginated.find(item => item._id === productId);
    ...
    if (wishlistBtn) {
      wishlistBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        toggleWishlistShop(productId, wishlistBtn, product);
      });
    }
  });
}
